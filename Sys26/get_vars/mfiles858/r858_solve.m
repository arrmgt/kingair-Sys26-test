function out = r858_solve(ptb, psa, dpa, dpb, dpr, dpn, f, pkor, sigma, alpha, opts)
%R858_SOLVE3  Joint weighted nonlinear least-squares solve for [q, ta, tb]
%using ALL 5 governing equations simultaneously.
% This properly distributes the 2
...
% fitting.
%   out = R858_SOLVE(ptb, psa, dpa, dpb, dpr, dpn, f, pkor, sigma, alpha, opts)
%
%   INPUTS (each scalar, or all the same size, e.g. Nx1 time series)
%     ptb, psa            Raw pressures; dp1 = ptb - psa internally
%     dpa, dpb, dpr, dpn   Measured differential pressures. 
...
%       least-squares fit 
%     - ANALYSIS MODE (sigma supplied): returns SE/CI/diagnostics too.
%
...
%     out.diag.iters  Gauss-Newton iterations used (should be small).
%                     since it's warm-started from the exact
