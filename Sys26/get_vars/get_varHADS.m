...
% ps_hads_a
ps_hads_a=get_hads(X,'A',HADSA_VR,HADSA_VT,HADSA_PER);

% ps_hads_b
ps_hads_b=get_hads(X,'B',HADSB_VR,HADSB_VT,HADSB_PER);
...
matfile=fullfile(X.tempdir,sprintf("%s_HADS.mat",X.BaseName));
delete(matfile)
...
sprintf('Processed get_varHADS.m for Project: %s',X.PROJ)