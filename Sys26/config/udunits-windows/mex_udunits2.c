...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    // Suppress unit override warnings from XML
    int old_stderr;
    ...
    if (!conv) {
        ut_free(fromUnit); ut_free(toUnit); ut_free_system(sys);
    ...
    mxArray *out = mxCreateNumericArray(mxGetNumberOfDimensions(prhs[0]), dims, mxDOUBLE_CLASS, mxREAL);
